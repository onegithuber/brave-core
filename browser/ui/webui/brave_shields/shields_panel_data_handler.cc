...
void ShieldsPanelDataHandler::GetSiteBlockInfo(GetSiteBlockInfoCallback callback) {
  std::move(callback).Run(site_block_info_.Clone());
}

void ShieldsPanelDataHandler::GetSiteSettings(GetSiteSettingsCallback callback) {
  auto* shields_data_ctrlr = GetActiveShieldsDataController();
  ...
  settings.is_https_everywhere_enabled = shields_data_ctrlr->GetHTTPSEverywhereEnabled();
