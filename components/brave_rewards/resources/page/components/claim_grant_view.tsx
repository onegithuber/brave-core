...
import { GrantInfo, formatGrantMonth, formatGrantExpirationDays } from '../../shared/lib/grant_info'
...
export function ClaimGrantView (props: Props) {
  ...
  const expirationDays = formatGrantExpirationDays(grantInfo)
            ...
            <div>
              {
                formatMessage(getString('rewardsGrantDaysRemaining'), [
                  <style.days key='days'>{expirationDays}</style.days>
                ])
              }
            </div>
        }
